...
🏷️ `[verified: สังเกตได้จริง 1 ครั้ง]` · **ขอบเขต**: engine เดียว (`sage-claude-oracle`) · **n=1**

**✅ สมมติฐานถูกแยกแล้ว โดยไม่ต้องรันซ้ำ** `[ajfon 2026-08-04 · หลักฐานเช้าวันเดียวกัน]` —
ผมตั้งไว้ 2 ข้อ (`hey` กด Enter เอง / engine auto-submit) · เขาล้มข้อแรกด้วยเคสที่เขามีอยู่แล้ว:
ยิง `hey -f` เข้า `role-classifier` (**codex**) แล้ว `peek` ทันที เห็น `[Pasted Content 1018 chars]`
**ค้างในช่องพิมพ์ ไม่มี busy marker** · ยิง `send-enter` แล้วจึงขึ้น `Working… esc to interrupt`
⇒ **ถ้า `hey` กด Enter เอง เคสนั้นต้อง submit ด้วย แต่มันไม่** ⇒ **เหลือคำอธิบายเดียว: ขึ้นกับ engine**

| engine | หลัง `maw hey -f` เปล่า ๆ | หลักฐาน |
|---|---|---|
| codex | ค้างในช่องพิมพ์ · **ต้อง `send-enter`** | ajfon n=1 |
| claude | **เข้า turn เอง** | ผม n=1 |
| opencode | **ยังไม่รู้** (ajfon เรียก `send-enter` ทุกครั้ง ไม่มีเคสควบคุม) | ⬜ |

⇒ ขั้นตอนของ ajfon **ไม่ผิด แต่เขียนกว้างกว่าหลักฐาน** · เขาแก้ `up-does-not-send-prompt.md` เอง
⇒ **`send-enter` ยังเป็นค่าตั้งต้นที่ถูก** (Enter บนช่องว่างไม่ทำอะไร) แต่เหตุผลเปลี่ยนจาก
   *"จำเป็นเสมอ"* → *"ถูกเสมอ แต่จำเป็นบางเครื่องยนต์"*
