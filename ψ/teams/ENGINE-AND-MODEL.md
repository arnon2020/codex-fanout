...
>
> 🔄 **และมันตัดกลับอีกทาง — alias ไม่ใช่ชั้นเดียวที่ตอบมิตินี้**
> `[verified 2026-08-08 · probe เอง]` boot `codex` **เปล่า ๆ ไม่มีแฟลกเลย** → สั่ง
> `curl https://example.com` + เขียนไฟล์ **นอก cwd** ⇒ **ทำทั้งสองอย่างโดยไม่ถามสักครั้ง**
> เพราะ `~/.codex/config.toml` ตั้ง `approval_policy = "never"` **ไว้ทั้งเครื่อง**
> 🔁 **วัดซ้ำแล้วบน `codex-cli 0.147.0`** `[verified 2026-08-08 ~23:5x]` — ผลเดิมยืน (curl 200 +
> เขียนไฟล์นอก cwd โดยไม่ถาม) · จำเป็นต้องวัดซ้ำเพราะ **binary ถูกอัปเกรดกลางเครื่องคืนนั้น**
> จาก 0.146.1 → 0.147.0 โดย pane ของทีมบ้านอื่นที่กด `1` บน update dialog เอง
> ⇒ `valid-if: codex --version` — ป้ายที่เขียนว่า 0.146.1 **วัดของที่ไม่มีอยู่แล้ว**
> ⇒ และ config **รอดการอัปเกรด** (ตรวจแล้ว `approval_policy` ยังอยู่) ซึ่งไม่ใช่สิ่งที่เดาได้
> ⇒ เครื่องมือที่อ่านแต่สตริงคำสั่งจะ **ตะโกนใส่ codex alias ที่ไม่มีปัญหา** —
> **false alarm ทิศ "กล่าวหา" ฆ่าเครื่องมือเตือนได้พอกับ false green**
> ⇒ `_vc_permmode` จึงอ่าน `$CODEX_HOME/config.toml` (fallback `~/.codex`) ก่อนตัดสินว่า `ask`
> ⏳ **แต่ config ที่ persist ไม่เดินทางไปกับ charter · alias เดินทางไปด้วย**
> ⇒ ย้ายเครื่อง/ส่งต่อเจ้าของ แล้ว**เกราะหายโดยที่ alias หน้าตาเหมือนเดิม**
> ⇒ **ใส่ token ใน alias อยู่ดี** — config คือเหตุผลที่จะไม่ตกใจ **ไม่ใช่คำตอบ**
> ⇒ ตรงกับกฎ `valid-if:` ของรีโปนี้เป๊ะ: claim ที่ผูกกับสภาพเครื่อง ต้องพกคำสั่งตรวจ ไม่ใช่แค่วันที่
>
...
5. 🆕 **`team resume` คืนทุก role กลับเป็น `claude` โดยไม่มี error**
   (`team_resume.rs:57-61` / `team_spawn.rs:89`) ⇒ **ทีมที่ spawn มาถูก engine ทุกตัว
   ยุบเป็น engine เดียวเงียบ ๆ เมื่อ resume** — คลาสเดียวกับข้อ 1-4 เป๊ะ: *ขอแล้วไม่ได้ โดยไม่มีเสียง*
   ⚠️ **atlas เป็นคนเห็นว่าอันนี้เป็นช่องว่างจริง ไม่ใช่ผม** — ผมเขียนพฤติกรรมนี้ไว้ในเอกสาร
   แต่ไม่ได้ตระหนักว่ามันคือช่องโหว่ใน `codex-team` ของเขา (โหมด `recover` ไม่เตือนเรื่องนี้)
   เขากำลัง fold เข้า **T4543** ฝั่งเขา `[2026-08-07 · owner decides · ผมไม่แตะ skill ของเขา]`
   📌 **ยังไม่ยื่น upstream** — ข้อ 1-5 ทั้งชุดยังไม่มีใบ (คนละเรื่องกับ **#785** ซึ่งเป็น
   *identity naming* ไม่ใช่ *engine resolution*) · ผมยืนยัน `:57-61` **ด้วยตาตัวเองไม่ได้**
   เพราะ checkout `maw-rs` บนเครื่องนี้คนละสายกับ binary ที่รัน ⇒ **ป้ายเป็นของ atlas
   ไม่ใช่ของผม** ก่อนยื่นต้องอ่าน source ที่ตรงกับ binary ด้วย `git show <sha>:<path>`

### 🔀 คนละเรื่องกับ engine — บันทึกไว้ไม่ให้หาย

**`maw-rs team up` ปฏิเสธ cwd ที่อยู่นอก repo root** (`invalid worktree path ... outside repo root`)
ขณะที่ **`maw-js team spawn` ยอมทุกที่** `[verified 2026-08-07 · prism · c1e8797]`
⇒ **ไม่ใช่บั๊กของ engine resolution และไม่ใช่ instance ของ #785** (ที่เป็นเรื่องการประกอบชื่อ)
⇒ ผมใส่ไว้ใน #785 **เป็นบริบทเท่านั้น** และระบุในใบว่าไม่ใช่ส่วนของคำขอให้แก้
⇒ **ตัดสินแล้วว่ายังไม่ยื่นแยก**: n=1 · ยังไม่รู้ว่าเป็นเจตนา (maw-rs เข้มขึ้น) หรือ regression
   จาก parity gap · **ถ้าบ้านที่สองเจอ ให้ยื่นทันที** — ถึงตอนนั้นมันจะมีหลักฐานว่าเป็นอย่างหลัง
