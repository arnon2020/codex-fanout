...
## 🔁 ตรวจซ้ำหลัง advisor ท้วง (ยังในรอบสอง)

**probe แรกผิดเครื่องมือ** — ผมกรอง `project: .../codex-fanout` ได้ 0 แล้วสรุป ทั้งที่
**`project:` ติดตาม*ผู้แบงก์* ไม่ใช่เจ้าของเรื่อง** (หลักฐานอยู่ในผลของผมเอง: doc เรื่องเรา
ถูกแบงก์ใต้ `project: .../ajfon-teams`) ⇒ filter นั้น**ตอบคำถามนี้ไม่ได้ตั้งแต่แรก**
และ 4 probe แรกเป็น vector top-N ล้วน (`ftsMatches: 0` ทุกอัน) — **ไม่ใช่การ enumerate**
⇒ ซ้ำรอย 2 commit ก่อนหน้าเป๊ะ: claim ทางลบที่หลักฐานไม่ครอบสโคปที่มันอ้าง

**probe ที่ตัดสินได้จริง** `[verified 2026-08-05]`

- `"teamclosed"` `mode: fts` → **`ftsMatches: 0`** ← คำที่เราประดิษฐ์เอง มีที่เดียวคืองานเรา
  ⇒ **หลักฐานชี้ขาด** งาน 08-03/08-04 ไม่อยู่ใน Arra จริง
- `"codex-fanout"` `mode: fts` → **16 hit** ทุกอันเป็นของ oracle อื่นที่พูดถึงเรา
- `"shopee"` `mode: fts` → **6 hit ใน 4ms** ⇒ **FTS ไม่ได้พัง**

**ข้อสรุปยืน แต่เหตุผลเปลี่ยน** — สิ่งที่ทำให้ยืนคือ distinctive-token FTS ไม่ใช่ `project:` filter

**ปิดป้าย `[unverified]` เรื่อง FTS**: FTS ทำงานปกติ · ที่ได้ 0 คือ **query ยาวหลายคำ/ภาษาไทย**
ตกไปใช้ vector หมด ⇒ **ก่อน dedup ก่อนแบงก์ ต้อง probe ด้วย token เดี่ยวที่โดดที่สุด**
query ยาวให้ความรู้สึกว่าค้นละเอียดกว่า แต่**ปิดช่องทางที่แม่นที่สุดทิ้ง**

**หมายเหตุ family**: `76+` เป็น**ข้อความในหัวเรื่อง** issue #60 ที่มนุษย์พิมพ์ ไม่ใช่จำนวนที่ query ได้
— พูดได้แค่ *"หัวเรื่องเขียนว่า 76+"* ไม่ใช่ *"registry มี 76"*
