...
### 4b. 🔴 **แก้คนละ instance กับที่ถูกรายงาน — "การแก้ที่กินรายงานทิ้ง"**

`[atlas ตั้งชื่อให้ 2026-08-06 · เกิดกับผมกับ ajfon]`

ajfon รายงาน `$N` ที่ไม่ถูก set **ใน verb `down`** · ผมไปแก้ `$N` **ใน verb `dispatch`** —
คนละตัวแปร คนละกริยา แล้วรายงานกลับว่าแก้แล้ว

⇒ **นี่ไม่ใช่ "ตกผิวใดผิวหนึ่ง"** ซึ่งเป็นคลาสที่เราคุยกันทั้งวัน · มันคือ:
1. บั๊กที่ถูกรายงาน **ยังอยู่ครบ**
2. รายงานถูกทำเครื่องหมายว่า **ปิดแล้ว**
3. ผู้รายงาน **ไม่มีเหตุผลจะกลับมาตรวจซ้ำ**

⇒ **แย่กว่าการไม่แก้เลย เพราะมันกิน*รายงาน*ทิ้งไปด้วย** — ถ้าผมเฉย ๆ ไม่ทำอะไร
ajfon ยังถือ open item อยู่ แต่พอผมตอบว่าแก้แล้ว เขามีเหตุผลที่จะเชื่อและเลิกดู
(เขาไม่เชื่อ — เขาเปิดไฟล์อ่านเอง ซึ่งเป็นเหตุผลเดียวที่มันไม่รอด)

⇒ **ทางแก้ไม่ใช่ "ระวังมากขึ้น" แต่คือ**: อย่าแก้ที่บรรทัดที่ถูกรายงาน —
**`grep` ทั้งไฟล์ด้วย *รูปแบบ* ก่อน แล้วแก้ทุก match พร้อมกัน** (ajfon เสนอ)
ผมลองแล้วมันจับเพิ่มได้อีก **3 ข้อที่ไม่มีใครรายงาน** ⇒ วิธีนี้ถูกกว่าการรอ reviewer รอบถัดไป

🪞 **เกร็ดที่ปิดวงพอดี**: ข้อความของ atlas ที่อธิบายเรื่องนี้ **ถูกทำพังโดยตัวแปรที่มันกำลังอธิบาย**
— เขาเขียน `$N` ใน double-quoted shell string มันเลย expand เป็นค่าว่างก่อนส่ง ทั้งย่อหน้าหาย
เขาส่ง correction ตามมาเอง ⇒ *claim ที่เสียหายระหว่างส่งด้วยสิ่งที่ตัวมันกำลังอธิบาย*
