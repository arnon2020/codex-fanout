...
### 🔴 Read the untracked file **names**, never just the count

`[verified 2026-08-07, lucifer, tearing down ws-parity-port]`

Before removing any worktree, print `git status --short` **in full** and read what is there.
A count is not a check. `dirty=11` and `dirty=0` look like a severity difference; they are
not — one of them was the entire deliverable.

```
wsparity-verify-ui   dirty=11   ← 8 screenshots + 2 JSON runs + 1 DOM dump
wsparity-coder-ui    dirty=1    ← .playwright-mcp/
```

Those eleven files were the **only human-visible reproduction of the defect anyone in the
fleet produced**: the UI kept rendering while the data stream went silent —
`quietIntervalMs 5679`, `domSampleCountDuringQuiet 22`, `domMaxGapMsDuringQuiet 277`.
The browser was not frozen; the stream was. Nothing errored, nothing crashed, the page
simply stopped being true for six seconds. No test asserts that, and no one can re-derive
it from the branch.

**`git status` called them untracked noise. They were the finding.** A teardown that reads
only the count deletes the most expensive evidence of the run, every run, and reports
success — because by its own measure nothing was tracked, so nothing was lost.

⇒ Rescue first, remove second. Copy anything untracked into the snapshot directory
**before** `worktree remove --force`, and say in the report what you rescued.

### 🔴 Removing a worktree must not endanger its branch — check the commits *after*

`[verified 2026-08-07, same teardown]` — **a separate lesson from the one above; do not
merge them.** That one is about files git ignores. This one is about commits git keeps, but
only if you let it.

One branch held **three commits that existed nowhere else** — a red test, the fix it
proved, and a fixture isolation — none of them merged anywhere, and parked pending a base
decision by another repo's owner:

```
agents/coder-a-ws-nonblocking-actions = b1748a7
  b1748a7  test(serve): isolate websocket status fixture
  26ee8f5  fix(serve): keep websocket actions off the async loop
  bab3727  test(serve): expose websocket action stalls
```

`worktree remove` does not delete a branch — but a teardown that pairs it with any branch
cleanup, or that lets `--clean` run over the same names, will. And the loss is silent:
nothing errors, and nobody discovers it until someone goes looking for the work.

⇒ Before removal: `git log --oneline <base>..<branch> | wc -l` for every branch a worktree
holds. ⇒ After removal: **re-run it and confirm the count is unchanged.** Verifying only
before proves you had them; verifying after proves you still do.
