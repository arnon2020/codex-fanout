...
- 📮 **retraction ที่ไม่มีช่องทาง durable = retraction ที่ไม่ได้เกิดขึ้น** (ajfon 2026-08-04)
  — correction/retraction **ต้องส่ง inbox file ควบเสมอ** ไม่พึ่ง tmux delivery อย่างเดียว
  ⇒ `delivered` **ไม่ใช่** `arrived` · **`ยังไม่ได้ส่ง` ก็เป็น claim ที่ต้อง `grep` เหมือน `ส่งแล้ว`**
  — ทิศลบไม่ได้ยกเว้นจากการตรวจ (2026-08-04: ผมประกาศว่ายังไม่ได้ส่ง D14 ให้ lucifer
  จากความจำ ทั้งที่ไฟล์ที่ผมเขียนเองอยู่ห่างไป 2 commit และมีอยู่ครบ)
  ⇒ **relay ที่ทิ้ง output ของตัวเอง (`>/dev/null 2>&1`) ไม่มีทางบอกได้ว่ามันล้ม**
